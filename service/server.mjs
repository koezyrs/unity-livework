...
export async function createLiveWork({ port = 8080, host = '127.0.0.1', code = String(randomInt(100000, 1000000)), hostToken = randomBytes(32).toString('hex'), persist = false } = {}) {
  ...
  if (persist) { await mkdir(path.join(root, '.local'), { recursive: true }); await writeFile(path.join(root, '.local/host.json'), JSON.stringify(config, null, 2)); }
  return { ...config, server, async close() {
    clearInterval(heartbeat); clearPending('Service stopped');
    for (const ws of wss.clients) ws.terminate();
    await new Promise(resolve => server.close(resolve));
    wss.close();
    if (persist) await unlink(path.join(root, '.local/host.json')).catch(() => {});
  } };
}

if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  const app = await createLiveWork({ port: Number(process.env.LIVEWORK_PORT || 8080), host: process.env.LIVEWORK_BIND || '0.0.0.0', persist: true });
