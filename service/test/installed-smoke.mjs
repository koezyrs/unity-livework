...
try {
  ...
  const result = { media: await page.locator('#video').evaluate(v => ({ width: v.videoWidth, height: v.videoHeight, tracks: v.srcObject.getTracks().map(t => t.kind) })), stats: await page.locator('#stats').textContent(), errors };
