...
try {
  ...
  if (await page.locator('#play').isEnabled()) await page.locator('#play').click(); await status('playing'); await videoReady();
  ...
  await page.locator('#preset').selectOption('1280x720'); await page.locator('#resize').click(); await until(s => s.width === 1280 && s.height === 720); await videoReady();
  ...
  await page.locator('#stop').click(); await status('stopped');
